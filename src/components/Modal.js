...
const Modal = ({ isOpen, onClose, onUpload }) => {
  ...
  const upload = async () => {
    ...
    if (video.videoFile != null) formData.append("videoFile", video.videoFile);
    if (video.imageFile != null) formData.append("imageFile", video.imageFile);
    ...
    console.log(response.data); // 추가된 비디오 정보
    onClose();
    onUpload(response.data); // 바로 추가된거 올리기
  };
  return (
